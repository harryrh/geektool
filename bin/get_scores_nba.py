...
def format_record(record, f):
    ...
    record_re = re.compile('\((\d+)-(\d+),\s+\d+-\d+ \w+\)')
...
conf_parser.add_argument('-s', '--section',  dest='section', default='nba',
...
parser.add_argument('--prefix', dest='prefix', default='nba-game-',
...
def get_team(header, boxid, homeaway, args):
    ...
    image = get_team_image(normalize(name), args)
